...
fn handle_client(stream: TcpStream) -> Result<(), Error> {
    ...
    loop {
        data.clear();
        // let bytes_read = stream.read_until(b'\n', &mut data).expect("hello???");
        let bytes_read = stream.read_until(b'\n', &mut data)?;
        if bytes_read==0 { return Ok(()); }
        let input:Point3D = serde_json::from_slice(&data)?;
        println!("{:?}", input);
        let val:u32 = input.x.pow(2)+input.y.pow(2)+input.z.pow(2);
        write!(stream.get_mut(), "{}", f64::from(val).sqrt())?;
...
fn main() {
    ...
    if args.len()!=2 {
        eprint!("Please provide --client or --server as argument");
        std::process::exit(1);
    }
    if args[1]=="--server" {
        ...
        loop {
            ...
            let parts:Vec<&str> = input.trim_matches(['\r', '\n']).split(',').collect();
            let point = Point3D {
            ...
            stream.write("\n".as_bytes()).expect("Failed to write to server");
            let mut reader = BufReader::new(&stream);
            reader.read_until(b'\n', &mut buffer).expect("Could not read into budder");
